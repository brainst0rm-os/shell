...
/**
 * Wires-up tests for the `/graph` slash command (Graph 9.13.12): the
 * command opens the shared embed picker scoped to `brainstorm/Graph/v1`,
 * and the picker store carries the type filter through. The picker
 * plugin's list-narrowing rides the same `typeFilter` field.
 */
...
describe("/graph slash command (block.embed.graph)", () => {
	const slash = BLOCK_COMMANDS.find((c) => c.id === "block.embed.graph");

	it("is registered under the Embed category with a 'graph' keyword", () => {
		expect(slash).toBeDefined();
		expect(slash?.category).toBe(CommandCategory.Embed);
		expect(slash?.keywords).toContain("graph");
	});

	it("opens the embed picker scoped to brainstorm/Graph/v1", () => {
		expect(slash).toBeDefined();
		...
		expect(target?.typeFilter).toBe("brainstorm/Graph/v1");
	});
