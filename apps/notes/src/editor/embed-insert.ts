 ...
 * are known) and the future cross-app drag-into-note path (where a
 * drop target identifies the host paragraph). Keeping the editor
 * mutation in pure helpers lets the entity-picker UI and the slash
 * wiring be tested without a DOM.
 *
 * The helper *replaces* a target paragraph (the now-empty `/<query>`
 * row left behind by the slash menu) — it does not insert after.
 * Falling back to root-append on a missing key mirrors `insertSubPage`'s
 * recovery path so a stale paragraph key never silently drops the embed.
 */
...
export type EmbedInsertion = {
	 ...
	 *  `services.blocks.forType(entityType)` (the providing app's live block),
	 *  falling back to the generic shell entity-card when omitted. */
	blockId?: string;
};
