 ...
 * When a providing app contributes a BP block for `blockId` (Stage 9.5
 * block-frame iframe + B6.4a live transclusion sub-editor), the same
 * persisted node lights up the live mount; the on-disk shape doesn't
 * change.
 *
 ...
 * target has loaded. Mirrors the same fields MentionNode/PageRefNode
 * already persist — keeping the shell-side body walker's edge extraction
 * a single-pass scan.
 */

import { dispatchOpenEntity, getEntityDisplayIcon } from "@brainstorm/editor";
...
import { entityIconsSnapshot, subscribeEntityIcons } from "../../store/entity-icon-index";
import {
...
/** The shell-provided generic preview block. Until a providing app ships
 *  its own BP block (B9.5 frame), every embed renders through this id —
 *  the fallback-renderer path called out in doc-15 §The bridge. */
...
export class BlockEmbedNode extends DecoratorNode<JSX.Element> {
	 ...
	 *  attribute value matches the exact stamp this app emits
...
export function BlockEmbedView({
	...
	const display = liveTitle?.trim() || label.trim() || t("notes.embed.untitled");
	const icon = getEntityDisplayIcon(entityId, entityType);
	const isBp = renderer?.kind === BlockRendererKind.BlockProtocol;
	const typeLabel =
		isBp && renderer ? blockProtocolSubtitle(renderer, entityType) : entityTypeLabel(entityType);
...
function blockProtocolSubtitle(renderer: { appId: string }, entityType: string): string {
	return t("notes.embed.providedBy", { type: entityTypeLabel(entityType), app: renderer.appId });
}
